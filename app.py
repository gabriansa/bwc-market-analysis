...
def create_plot(df, x_axis, y_axis):
    ...
    # Define specific order for MSI Familiarity categories
    x_categories = [
        "Far Adjacency",
        "Mid Adjacency",
        "Near Adjacency", 
    ...
    fig = px.scatter(
        ...
        custom_data=['Occupation', x_axis, y_axis, 'Employment', 'Small Form Factor'],
        title="BWC Familiarity of Occupation vs MSI Familiarity of Occupation by BLS Category",
        labels={
            'BLS Category': 'Occupational Category',
            'Employment': 'Total Employment',
            'x_jittered': 'MSI Familiarity of Occupation',
    ...
    fig.update_traces(
        hovertemplate=(
            ...
            "<b>MSI Familiarity:</b> %{customdata[1]}<br>"
            "<b>BWC Familiarity:</b> %{customdata[2]}<br>"
            "<b>Small Form Factor:</b> %{customdata[4]}<br>"
...
def main():
    ...
    # Basic description
    st.markdown("""
    This dashboard shows the relationship between MSI Familiarity and BWC Familiarity across different occupational categories.
    ...
    if selected_form_factors:
        df = df[df['Small Form Factor'].isin(selected_form_factors)]
    ...
    """)
    st.plotly_chart(create_plot(df, y_axis='BWC Familiarity of Occupation', x_axis='MSI Familiarity of Occupation'), use_container_width=True)
